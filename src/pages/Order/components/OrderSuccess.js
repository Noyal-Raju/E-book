  ...
  return (
    <section className="text-xl text-center max-w-4xl mx-auto my-10 py-5 dark:text-slate-100 border dark:border-slate-700 rounded">
        <div className="my-5">
            ...
            <p>Thank you <span className="font-semibold uppercase">{data.user.name}</span> for the order!</p>
            <p>Your Order ID: {data.id}</p>          
        </div>
        <div className="my-5">
            <p>Your order is confirmed.</p>
            <p>Please check your mail <span className="font-semibold">({data.user.email})</span> for the eBook.</p>
            <p className="my-5">Payment ID: {data.user.payment_id}</p>
        </div>
